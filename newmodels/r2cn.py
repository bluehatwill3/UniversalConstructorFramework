...
def run_apex_training():
    ...
    for epoch in range(1, epochs + 1):
        ...
        feat_basic = torch.tensor([[ac["geo_prec"], ac["asm_eff"], current_energy/10000.0, 0.1, 0.1]], dtype=torch.float32).to(DEVICE)
        ...
        # Action Physics
        energy_loss = 0
        if student_action == 0: energy_loss = 120.0
        elif student_action == 1: energy_loss = 60.0
        ...
        if epoch % 25 == 0:
            print(f"  [Epoch {epoch:03d}] Act: {student_action} (Teach: {teacher_action}) | NRG: {current_energy:4.0f} | Attn: {attention_signal:.2f} -> Rwd: {final_reward:+.1f}")

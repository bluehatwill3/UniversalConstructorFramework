...
# ─────────────────────────────────────────────────────────────────────────────
#  PATHS  (update to match your local layout)
# ─────────────────────────────────────────────────────────────────────────────
MODEL_DIR = os.path.join(os.path.dirname(__file__), "models")

HOLOSYN_TORCHSCRIPT   = os.path.join(MODEL_DIR, "1772300642283_holosyn_heads_torchscript.pt")
HOLOSYN_STATEDICT     = os.path.join(MODEL_DIR, "1772300642285_holosyn_heads.pt")
STUDENT_HF_TS_1       = os.path.join(MODEL_DIR, "1772300642277_student_distilled_heads_hf_torchscript__1_.pt")
STUDENT_HF_TS_2       = os.path.join(MODEL_DIR, "1772300642282_student_distilled_heads_hf_torchscript.pt")
STUDENT_BASIC_TS      = os.path.join(MODEL_DIR, "1772300642281_student_distilled_heads_torchscript.pt")
NORM_JSON             = os.path.join(MODEL_DIR, "1772300642283_student_norm_hf.json")
...
class UniversalConstructor:
    ...
    def __init__(
        ...
        # ── Load models ──────────────────────────────────────────────────
        print("\n📦 Loading models...")
        self.normalizer    = FeatureNormalizer(os.path.join(mdir, "1772300642283_student_norm_hf.json"))
        self.holosyn       = HoloSynHeads()
        self.student_hf    = StudentDistilledHeadsHF()
        self.student_basic = StudentDistilledHeadsBasic()

        hs_path = os.path.join(mdir, "1772300642283_holosyn_heads_torchscript.pt")
        hf_path = os.path.join(mdir, "1772300642282_student_distilled_heads_hf_torchscript.pt")
        bs_path = os.path.join(mdir, "1772300642281_student_distilled_heads_torchscript.pt")
...
def load_production_stack(model_dir: str) -> Tuple[PerceptionPipeline, Dict]:
    ...
    """
    norm    = FeatureNormalizer(os.path.join(model_dir, "1772300642283_student_norm_hf.json"))
    holosyn = HoloSynHeads()
    student_hf    = StudentDistilledHeadsHF()
    student_basic = StudentDistilledHeadsBasic()

    for cls, path in [
        (holosyn,       os.path.join(model_dir, "1772300642283_holosyn_heads_torchscript.pt")),
        (student_hf,    os.path.join(model_dir, "1772300642282_student_distilled_heads_hf_torchscript.pt")),
        (student_basic, os.path.join(model_dir, "1772300642281_student_distilled_heads_torchscript.pt")),
    ]:

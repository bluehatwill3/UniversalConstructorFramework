...
║  Modulation (R-STDP), Homeostasis, Acetylcholine (Attention), Serotonin      ║
║  (Mood/Threshold), and Heterosynaptic Competition.                           ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
try:
    ...
    class WrenchStamped: pass
    class JointTrajectory: pass
    class JointTrajectoryPoint: pass
    class CvBridge: pass
...
class AdvancedNeuromorphicHead(nn.Module):
    ...
    Integrates STDP, Reward Traces, Homeostasis, Attention, Mood, and Competition.
    """
    def __init__(self, n_actions: int):
        ...
        # INDEPENDENT MODULATION: Serotonin/Mood modifies the firing threshold dynamically
        eqs_neurons = '''
        dv/dt = (I - v) / (10*ms) : 1
        I : 1
        mood_modifier : 1  # Raises/lowers threshold based on risk perception
        '''
        self.G = b2.NeuronGroup(n_actions, eqs_neurons, threshold='v > (1.0 + mood_modifier)', reset='v=0', refractory=2*b2.ms, method='euler')

        # ── THE ADVANCED BIOLOGICAL LEARNING EQUATIONS ──
        syn_eqs = '''
        ...
        # THE MASTER LEARNING RULE COMBINATIONS:
        # 1. Combinatory (R-STDP + ACh): (base_lr * attention) * c * reward
        # 2. Independent (Homeostasis): - w_decay * w
        # 3. Semi-Independent (Heterosynaptic Competition): - hetero_decay * global_activity * w
        dw/dt = (base_lr * attention) * c * reward - w_decay * w - hetero_decay * global_activity * w : 1 (clock-driven)
        ...
        # Dynamic Modulators Initialization
        self.S.reward = 0.0
    ...
    def predict_and_learn(self, z: torch.Tensor, reward_signal: float, attention_signal: float, mood_signal: float) -> int:
        """Pushes data through the SNN, applies multi-factor modulation, and acts."""
        ...
        # Calculate Heterosynaptic global activity (proxy for systemic excitation)
...
class CyberPhysicalNode(Node):
    def __init__(self):
        ...
        self.pub_arm = self.create_publisher(JointTrajectory, '/joint_trajectory_controller/joint_trajectory', 10) # Arm Controller
    ...
    def cognitive_loop(self):
        ...
        # 3. SNN Inference & Multi-Factor Plasticity Update
        ...
        # 4. Amygdala Safety Check & Environmental Deltas
        ...
        # --- DYNAMIC NEUROMODULATION CALCULATION ---

        # Acetylcholine (Attention): Spikes when sensory delta is high
        self.last_attention = 1.0 + min(2.0, abs(temp_delta) * 0.5)

        # 5. Actuate & Generate Next Reward/Mood Traces
        if is_safe:
            # Serotonin (Mood): Lowers threshold slightly during safe exploration
            self.last_mood = -0.1
            ...
            self.get_logger().info(f"✅ Safe Action Executed: {proposed_action} | Attention: {self.last_attention:.2f}")
            if not HAS_ROS2: self.current_temp += 5.0
        else:
            # Serotonin (Mood): Raises threshold significantly, making neurons conservative/risk-averse
            self.last_mood = +0.5

            # Penalty Bypass (Homeostatic forgetting)
            self.last_reward = 0.0
            self.get_logger().warn(f"🛑 BLOCKED ({reason}): Threshold Raised. Dopamine Neutralized.")

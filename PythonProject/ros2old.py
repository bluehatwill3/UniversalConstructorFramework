...
class AdvancedNeuromorphicHead(nn.Module):
    ...
    def __init__(self, n_actions: int, learning_modules: dict = None):
        ...
        self.S.connect()
        # Give a stronger random initialization so we can see decay/growth clearly
        self.S.w = 'rand() * 0.3'
        self.S.x = 1.0
        ...
        self.S.base_lr = 1.5 * b2.Hz  # Boosted for faster offline demonstration

        # Init Modular Variables safely
        if self.modules["homeostasis"]: self.S.w_decay = 0.02 * b2.Hz
        if self.modules["heterosynaptic"]:
            self.S.hetero_decay = 0.05 * b2.Hz  # Strong pruning
            self.S.global_activity = 0.0
        if self.modules["predictive_coding"]:
            self.S.mech_lr = 2.0 * b2.Hz  # Aggressive mechanical correction
    ...
    def predict_and_learn(self, z: torch.Tensor, reward_signal: float, attention_signal: float, mood_signal: float,
                          mech_error_signal: float, force_hetero_surge: bool = False) -> int:
        ...
        if self.modules["heterosynaptic"]:
            if force_hetero_surge:
                # Bypass penalty: Artificial surge to wipe bad synapses instead of using dopamine
                self.S.global_activity = 10.0
            else:
...
class CyberPhysicalNode(Node):
    ...
    def cognitive_loop(self):
        ...
        if self.last_action_taken in [0, 1]:
            expected_delta = 5.0  # Actions generate heat
        elif self.last_action_taken == 2:
            expected_delta = -5.0  # Action 2 is 'Rest/Cool'
        elif self.last_action_taken == -1:
            expected_delta = -15.0  # Forced stop expectation

        # Only learn mechanics from our OWN actions, not governor interventions
        mech_error_signal = 0.0
        if self.last_action_taken != -1:
            mech_error_raw = actual_temp_delta - expected_delta
            # Positive error = Unexpectedly hot. This triggers predictive weight reduction.
        ...
        proposed_action = self.head.predict_and_learn(
            ...
            force_hetero_surge=self.force_prune_surge
        )
        self.force_prune_surge = False  # Reset flag
        ...
        if is_safe:
            ...
            if proposed_action in [0, 1]:
                # Active movement commands generate heat
                if HAS_ROS2: self.pub_base.publish(Twist())
                if not HAS_ROS2: self.current_temp += 5.0
            elif proposed_action == 2:
                # "Rest" action cools the system actively
                if not HAS_ROS2: self.current_temp = max(40.0, self.current_temp - 5.0)

            err_str = f"| Mech Error: {mech_error_signal:.2f}" if self.head.modules.get("predictive_coding") else ""
            self.get_logger().info(f"✅ Safe Action: {proposed_action} {err_str}")
        else:
            self.last_mood = +0.5

            # ALTERNATIVE PENALTY ARRANGEMENT:
            # Bypass Dopamine penalty entirely (0.0). Instead, trigger a Heterosynaptic Prune Surge!
            self.last_reward = 0.0
            self.force_prune_surge = True
            self.last_action_taken = -1

            self.get_logger().warn(
                f"🛑 BLOCKED ({reason}): Bypassing Dopamine. Triggering Heterosynaptic Pruning Surge to wipe bad habits.")
...
def run_offline_tournament():
    print("\n" + "═" * 70)
    print(" 🏆 PLANET FACTORY: CONTINUAL LEARNING COMBINATIONS TOURNAMENT")
    print("═" * 70)

    configs = [
        ("TEST 1: Classic RL (Reward Only - Prone to getting stuck)", {
            "stdp": True, "reward": True, "attention": False,
            "homeostasis": False, "heterosynaptic": False, "predictive_coding": False
        }),
        ("TEST 2: Future-Proof Brain (Multi-Factor + Penalty Bypass)", {
            "stdp": True, "reward": True, "attention": True,
            "homeostasis": True, "heterosynaptic": True, "predictive_coding": True
        })
    ]

...
def run_apex_training():
    ...
    except Exception as e: print(f"⚠️ Teacher offline: {e}")
    ...
    for epoch in range(1, epochs + 1):
        # ── SENSORY FETCH ──
        ac = data.autocad_data[epoch % len(data.autocad_data)] if data.autocad_data else {"geo_prec": 0.5, "asm_eff": 0.5, "prec_chg": 0.0}

        # If DB is offline, we use our local current_energy tracker
        econ_energy = data.economy_data[epoch % len(data.economy_data)]["energy"] if data.economy_data else current_energy

        # Inject periodic "Precision Shock" to test attention
        if epoch % 50 == 0: ac["prec_chg"] = 0.8

        feat_basic = torch.tensor([[ac["geo_prec"], ac["asm_eff"], econ_energy/10000.0, 0.1, 0.1]], dtype=torch.float32).to(DEVICE)
        ...
        if student_action == 0: current_energy -= 100.0
        elif student_action == 1: current_energy -= 50.0
        else: current_energy += 200.0 # Recovery
        ...
        if epoch % 25 == 0:
            print(f"  [Epoch {epoch:03d}] Act: {student_action} (Teach: {teacher_action}) | NRG: {econ_energy:4.0f} | Attn: {attention_signal:.2f} -> Rwd: {final_reward:+.1f}")

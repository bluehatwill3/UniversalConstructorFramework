...
class UniversalConstructor:
    ...
    def run(self):
        ...
        print("\n📥 Phase 1: Distilling Open-Source Foundation Models...")
        teacher = FoundationTeacherAdapter(model_name="openvla/openvla-7b", use_mock=True)
        distiller = RoboticsDistiller(teacher)

        # Generate a synthetic curriculum representing real-world edge cases
        curriculum = [self.perception.make_synthetic_bundle(seed=i) for i in range(16)]

        for name, robot in self.robots.items():
            distiller.distill_to_robot(robot, curriculum, epochs=15)
        # ────────────────────────────────────────────────────────────────────

        # ── PHASE 2: SIMULATION ─────────────────────────────────────────────
        print(f"\n🚀 Phase 2: Starting Safety-Guarded Simulation — {self.n_cycles} cycles")
        ...
        for cycle in range(1, self.n_cycles + 1):
            for name, robot in self.robots.items():
                ...
                log_entry = {
                    "cycle": cycle, "action": action_name, "safe": is_safe,
                    "reason": reason, "mood": sentiment.mood
                }
                robot.action_log.append(log_entry)
                self.hive_log.append({"robot": name, **log_entry})

                if self.verbose:
                    status_str = f"Temp: {metrics['temperature']:.1f}°C" if is_safe else f"Temp: {metrics['temperature']:.1f}°C"
                    print(f"  Cycle {cycle:03d} | {robot.domain_cfg['emoji']} {name:<12} | {action_name:<20} | {status_str}")
# ═════════════════════════════════════════════════════════════════════════════
#  ENTRY POINT
# ═════════════════════════════════════════════════════════════════════════════

...
def run_training_engine():
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ── SENSORY ENCODING ──
        feat_array = [[ac_row["geo_prec"], ac_row["asm_eff"], econ_row["energy"] / 10000.0, econ_row["ore"] / 1000.0,
                       econ_row["goods"] / 100.0]]
        ...
        if epoch % 20 == 0:
            print(
                f"  [Epoch {epoch:03d}] Act: {action} | Geo: {ac_row['geo_prec']:.2f} | NRG: {econ_row['energy']:4.0f} | Attn: {attention_signal:.1f} -> Rwd: {step_reward:+.1f}")

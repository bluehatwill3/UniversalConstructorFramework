...
║  actions it cannot afford, organically learning supply-chain bottlenecks.    ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LivingPlanetNode(Node):
    ...
    def agent_action_cb(self, msg):
        ...
        # Deduct resources since the node proved it could afford them before publishing
...
CHASSIS_CONFIGS = {
    ...
    "Orbital_Lander": {
        ...
        "modes": {
            "Active": {
                # High cost actions! If the planet can't afford this, the agent is punished with Noradrenaline.
...
class StandardizedSwarmNode(Node):
    ...
    def cognitive_loop(self):
        ...
        econ_senses = torch.tensor([[
            min(1.0, self.planet_tele[0] / 10000.0),  # Energy
            min(1.0, self.planet_tele[1] / 1000.0),  # Ore
            min(1.0, self.planet_tele[2] / 100.0),  # Semi-Finished
            min(1.0, self.planet_tele[3] / 100.0),  # Goods
            min(1.0, self.planet_tele[4] / 500.0)  # Propellant
        ]], dtype=torch.float32)
        ...
        # High demand on Bloom drives high attention
        ...
        can_afford = (self.planet_tele[0] >= rec[1] and  # Energy
                      self.planet_tele[1] >= rec[2] and  # Ore
                      self.planet_tele[2] >= rec[3] and  # Semi
                      self.planet_tele[3] >= rec[4] and  # Goods
                      self.planet_tele[4] >= rec[5])  # Propellant
        ...
        if is_valid:
            ...
            # Noradrenaline Punishment for Thermal Limits OR Bankruptcy!

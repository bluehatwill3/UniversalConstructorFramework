...
class VisionEncoder(nn.Module):
    """
    Computer Vision Module: Processes raw images (pixels) into 2048-dim embeddings
    expected by the HoloSynHeads image projector.
    Builds a raw CV pipeline instead of relying solely on pre-computed features.
    """
...
class PerceptionTrainer:
    """
    Trains the Computer Vision models to align with the teacher projections.
    Simulates self-supervised contrastive learning (aligning CV with state space).
    """

    def __init__(self, vision_encoder: VisionEncoder, holosyn: 'HoloSynHeads', lr=1e-3):
        self.vision_encoder = vision_encoder
        self.holosyn = holosyn
        # Train both the raw CNN and the image projector head
        self.optimizer = torch.optim.Adam(
            list(self.vision_encoder.parameters()) + list(self.holosyn.p_i.parameters()),
            lr=lr
        )
        self.criterion = nn.MSELoss()

    def simulate_training_epoch(self, steps=20):
        print("\n  👁️  Initiating Computer Vision Perception Training...")
        self.vision_encoder.train()
        self.holosyn.p_i.train()
        total_loss = 0.0

        for step in range(steps):
            self.optimizer.zero_grad()

            # Simulate raw images: batch of 8, 3 channels, 224x224
            raw_images = torch.randn(8, 3, 224, 224)
            # Simulate target multimodal context (e.g., text/goal representations)
            target_z = F.normalize(torch.randn(8, 256), p=2, dim=-1)

            # 1. CV extracts 2048-dim features from pixels
            E_i = self.vision_encoder(raw_images)

            # 2. HoloSynHeads Image Projector maps to 256-dim fused space
            z_i = self.holosyn.p_i(E_i)

            # 3. Compute alignment loss
            loss = self.criterion(z_i, target_z)
            loss.backward()
            self.optimizer.step()

            total_loss += loss.item()
            if (step + 1) % max(1, steps // 4) == 0:
                print(f"      [Batch {step + 1:02d}/{steps}] Vision Alignment Loss: {loss.item():.4f}")

        print(f"  ✅ Vision Training Complete. Avg Loss: {total_loss / steps:.4f}")
...
class ModalityBundle:
    ...
    E_i: Optional[torch.Tensor]  # (1, 2048) image embedding
    ...
    raw_image: Optional[torch.Tensor] = None  # (1, 3, H, W) raw pixel data
...
class PerceptionPipeline:
    ...
    def perceive(self, bundle: ModalityBundle) -> Tuple[torch.Tensor, torch.Tensor, Dict]:
        ...
        """
        # 0. Process raw computer vision if provided and embedding is missing
        if bundle.E_i is None:
            if bundle.raw_image is not None and self.vision_encoder is not None:
                ...
                # Fallback to zeros if neither embedding nor raw pixels are available
    ...
    def make_synthetic_bundle(
        ...
        # Simulate raw camera pixels (1, 3, 224, 224) if using vision encoder
...
class UniversalConstructor:
    ...
    def __init__(
        ...
        if self.train_cv:
            ...
            trainer = PerceptionTrainer(self.vision_encoder, self.holosyn)
            trainer.simulate_training_epoch(steps=20)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train-cv", action="store_true",
                        help="Train and build the raw Computer Vision models prior to simulation")

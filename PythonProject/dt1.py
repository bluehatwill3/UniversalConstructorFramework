...
from ucf import (
    ...
    ModalityBundle, NORM_JSON, DEVICE
)
...
class FactoryDigitalTwin:
    ...
    def apply_action(self, action_id: int):
        ...
        # Action Map: 0: Move Left/Down, 1: Move Right/Up, 2: Maintain
        target_vel = 0.0
        if action_id == 0:
            target_vel = -0.5  # Sweep left
        elif action_id == 1:
            target_vel = 0.5  # Sweep right
    ...
    try:
        ...
        while True:  # Run until user closes the window
            ...
            bundle = ModalityBundle(
                ...
                E_i=torch.from_numpy(live_img).unsqueeze(0).flatten(1)[:, :2048],  # Mock Vision Encoder output
            ...
            action_id = ai_arm.action_head.predict(fused_emb)

            action_name = ai_arm.domain_cfg["action_map"].get(action_id, "Unknown")

            # --- ACT ---
            # Execute in the physics engine
            env.apply_action(action_id)

            if cycle % 100 == 0:
                print(f"  [Cycle {cycle}] Vision & Haptics processed -> SNN Decided: {action_name}")

...
class AdvancedNeuromorphicHead(nn.Module):
    def __init__(self, n_actions: int, neuro_config: SNNNeuroConfig):
        ...
        taupre:second (shared); taupost:second (shared); tau_c:second (shared); tau_d:second (shared); base_lr:1/second (shared); reward:1 (shared); attention:1 (shared); w_decay:1/second (shared); hetero_decay:1/second (shared); global_activity:1 (shared); mecherror:1 (shared); mech_lr:1/second (shared); panicsignal:1 (shared); panic_lr:1/second (shared)
        '''

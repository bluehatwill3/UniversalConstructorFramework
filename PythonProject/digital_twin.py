...
class FactoryDigitalTwin:
    def __init__(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"❌ Failed to load 3D URDF models. Ensure pybullet_data is properly installed. Details: {e}")
...
def run_embodied_simulation():
    ...
    try:
        ...
        while True:  # Run until user closes the window
            ...
            if cycle % 100 == 0:
                print(
                    f"  [Cycle {cycle}] Temp: {env.simulated_temp:.1f}C | GeoPrec: {current_geo_prec:.2f} -> AI Decided: {action_name}")

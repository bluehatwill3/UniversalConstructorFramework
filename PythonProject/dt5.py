...
║                                                                              ║
║  *UPGRADED*: Now ingests real AutoCAD telemetry and pre-trained .pt weights. ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class FactoryDigitalTwin:
    def __init__(self):
        ...
        # Validate and Load the factory floor and a KUKA robotic arm
        try:
            self.planeId = p.loadURDF("plane.urdf")
            self.robotId = p.loadURDF("kuka_iiwa/model.urdf", [0, 0, 0], useFixedBase=True)
        except p.error as e:
            raise RuntimeError(f"❌ Failed to load 3D URDF models. Ensure pybullet_data is properly installed. Details: {e}")
    ...
    def get_robot_camera_feed(self) -> np.ndarray:
        ...
        # Validation: Ensure standard ImageNet shape for our VisionEncoder
        if img.shape != (3, 224, 224):
            raise ValueError(f"📷 Vision validation failed! Expected shape (3, 224, 224), got {img.shape}")
    ...
    def get_joint_states(self) -> np.ndarray:
        ...
        # Validation: Ensure SNN haptic constraint is met
        if haptic_array.shape != (32, 3):
            raise ValueError(f"🦾 Haptics validation failed! Expected shape (32, 3), got {haptic_array.shape}")

        return haptic_array

    def apply_action(self, action_id: int):
        """Translates discrete SNN output to physical motor torques."""
        # Validation: Ensure the Neural Network output is strictly a discrete integer
        if not isinstance(action_id, (int, np.integer)):
            raise TypeError(f"⚠️ Action Validation Error: Expected integer ID, got {type(action_id)}")
    ...
    def update_hud(self, action_name: str, is_safe: bool, temp: float, geo_prec: float = 0.0):
        ...
        status = f"AI: {action_name} | Temp: {temp:.1f}C | Prec: {geo_prec:.2f}"
...
def run_embodied_simulation():
    ...
    # ── 3. LOAD PRE-TRAINED MATURE WEIGHTS ──
    weights_path = "robot_arm_snn_head.pt"
    if os.path.exists(weights_path):
        print(f"\n🧠 Injecting mature pre-trained weights from {weights_path}...")
        try:
            ai_arm.action_head.load_state_dict(torch.load(weights_path, map_location=DEVICE))
            print("✅ Mature weights integrated successfully! Amnesia bypassed.")
        except Exception as e:
            print(f"⚠️ Architecture mismatch during weight load: {e}. Falling back to random initialization.")
    else:
        print(f"\n⚠️ {weights_path} not found. SNN starting with random amnesiac initialization.")

    # ── 4. INGEST AUTOCAD DATA PIPELINE ──
    autocad_metrics = []
    csv_path = "autocad_data.csv"
    if os.path.exists(csv_path):
        print(f"📊 Ingesting engineering telemetry from {csv_path}...")
        with open(csv_path, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                try:
                    autocad_metrics.append({
        ...
        print(f"✅ Loaded {len(autocad_metrics)} frames of AutoCAD telemetry.")
    else:
        print(f"⚠️ {csv_path} not found. Operating blind (Mock scalar data will be used).")

    # 5. Connect to the 3D Physics Environment
    env = FactoryDigitalTwin()

    # 6. Pre-flight System Validation
    print("\n🔍 Running System Diagnostics & Shape Validation...")
    try:
        test_img = env.get_robot_camera_feed()
        test_haptics = env.get_joint_states()
        assert test_img.shape == (3, 224, 224), "Vision shape mismatch."
        assert test_haptics.shape == (32, 3), "Haptics shape mismatch."
        print("✅ System Validation Passed. Sensors and Neural Pathways nominal.")
    except Exception as e:
        print(f"❌ CRITICAL: System Validation Failed -> {e}")
        p.disconnect()
        return
    ...
    try:
        ...
        while True: # Run until user closes the window
            ...
            # --- SENSE: DATA PIPELINE (AutoCAD) ---
            current_geo_prec = 0.0
            if autocad_metrics:
                # Loop through the CSV data synchronously with the physics engine
                idx = (cycle - 1) % len(autocad_metrics)
                row = autocad_metrics[idx]
                current_geo_prec = row["geo_prec"]

                # Assemble the 5-Dimensional basic feature tensor
                norm_temp = env.simulated_temp / 100.0 # Normalize thermal mass
                feat_array = [[row["geo_prec"], row["asm_eff"], row["prec_chg"], norm_temp, 0.0]]
                feat_basic_tensor = torch.tensor(feat_array, dtype=torch.float32).to(DEVICE)
            else:
                feat_basic_tensor = torch.randn(1, 5).to(DEVICE)
            ...
            bundle = ModalityBundle(
                ...
                feat_basic=feat_basic_tensor, # <--- Injected real AutoCAD data
                mask=torch.ones(1, 4),
            )

            # --- THINK & SAFETY CHECK ---
            # Pass physical & tabular data through the Spiking Neural Network
            ...
            env.update_hud(action_name, is_safe, env.simulated_temp, geo_prec=current_geo_prec)

            if cycle % 100 == 0:
                print(f"  [Cycle {cycle}] Temp: {env.simulated_temp:.1f}C | GeoPrec: {current_geo_prec:.2f} -> AI Decided: {action_name}")
        ...
        print(f"\n⚠️ Runtime Exception encountered: {e}")
    finally:
        print("🔌 Disconnecting Digital Twin Physics Engine...")
        p.disconnect()
